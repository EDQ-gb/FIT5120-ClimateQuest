// Vercel Serverless Function (Node): proxy /api/scene/* to backend.
// Root-level copy so it works even if Vercel Root Directory is repo root.
...
export default async function handler(req, res) {
  try {
    ...
    const marker = '/api/scene/'
    const idx = raw.indexOf(marker)
    const tail = idx >= 0 ? raw.slice(idx + marker.length) : ''
    const url = `${BACKEND_BASE}/api/scene/${tail}`.replace(/\/+$/, '')
